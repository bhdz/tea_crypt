...
window.xxtea_crypto = (function () {
    ...
    var util = {
        ...
        //
        InitializeBlock: function (block, block_size, cleartext, from_position, padding) {
            var is_lastblock = false, j = 0, r = 0, p, alpha_code = "", code_point = "";
            if ((typeof block) === "object" && Array.isArray(block)) {
            
                for (j=from_position; j<from_position + block_size && j < cleartext.length; j += 4) {
                    r = 0;
                    
                    A = cleartext.charCodeAt(j); 
                    B = cleartext.charCodeAt(j + 1);
                    C = cleartext.charCodeAt(j + 2);
                    D = cleartext.charCodeAt(j + 3);
                    r = parseInt(A) << 24 | parseInt(B) << 16 | parseInt(C) << 8 | parseInt(D);
                    block.push(r);
                }
            }
        ...
        StringToBlocks2: function (cleartext, padding, block_size) {
            ...
            for (i = 0; i < cleartext.length; i += 1) {
                if ((i) % block_size == 0) {
                    r = 0;
                    block = util.CreateBlock(block_size);
                    last_block = util.InitializeBlock(block, block_size, cleartext, i, padding);
                    /*
                    to initializeblock it goes!
                    for (j = 0; j < block_size && j < i; j += 1) {
                        code_point = cleartext.charCodeAt(i +  j); 
                        alpha_code = cleartext.charAt(i+ j);
                        p = parseInt(code_point, 10);
                        r = r | p << (8 * block_size );
                        console.log("r = "+r.toString());
                        console.log("p = "+p.toString());
                        console.log("code_point = "+code_point.toString());
                        console.log("alpha_code = "+alpha_code);
                        console.log("j = "+j);
                        console.log("----");
                    }
                    b += 1;
