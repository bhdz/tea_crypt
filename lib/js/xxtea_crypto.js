...
window.xxtea_crypto = (function () {
    ...
    var util = {
        ...
        InitializeBlock: function (block, block_size, cleartext, from_position, padding) {
            var is_lastblock = false, block_index = 0, j = 0, r = 0, A = 0, B = 0, C = 0, D = 0;

            if ((typeof block) === "object" && Array.isArray(block)) {
            
                //Sanity check:: If it doesn't fit, then this _is_ the last_block in the message
                if (block_size < block.length * 4) {
                    is_lastblock =  true;
                } else {
                
                    for (j=from_position; j < from_position + block_size && j < cleartext.length; j += block_size/4) {
                        r = 0;
                    ...
                    if (j >= cleartext.length) {
                        j -= block_size/4 + from_position; //step: ->> back !and add: ->> padding
