...
window.xxtea_crypto = (function () {
    ...
    var util = {
        ...
        // block ** to be read
        // block_size ** in bytes
        // text ** the text to be blockified... ah yes padding
        // from_position ** position in the text
        ...
        //
        ReadBlock: function (block, block_size, text, from_position, padding) {
            ...
            if ((typeof block) === "object" && Array.isArray(block)) {
                ...
                if (block_size < block.length * util.BYTES_PER_BLOCK) {
                    ...
                    for (j=from_position; j < from_position + block_size && j < text.length; j += block_size / util.BYTES_PER_BLOCK ) {
                        r = 0;
                        
                        A = parseInt(text.charCodeAt(j),       10); 
                        B = parseInt(text.charCodeAt(j + 1),   10);
                        C = parseInt(text.charCodeAt(j + 2),   10);
                        D = parseInt(text.charCodeAt(j + 3),   10);
                        r = A << 24 | B << 16 | C << 8 | D;
        ...
        // block    ** to be written
        // text     ** into what the block is to be written
        // position ** starting position
        WriteBlock: function (block, text, position) {
            var index = 0;
            if ((typeof block) === "object" && Array.isArray(block)) {
                block.forEach(
                    function (currentValue, index, array) {
                        ...
                        // damn JS is not C, one cannot mutate strings.
                        //text[position + index] = String.fromCharCode(A);
                        //text[position + index + 1] = String.fromCharCode(B);
                        //text[position + index + 2] = String.fromCharCode(C);
                        //text[position + index + 3] = String.fromCharCode(C);
                        index += 1;
                    }
                );
            }
        ...
        // text ** what is to be encrypted
        // padding   ** what is to be appended when string.length % 4
        // block_size ** in bytes
        StringToBlocks: function (text, padding, block_size) {
            ...
            //add padding here:: based on <text.length % block_size >
            
            for (i = 0; i < text.length; i += 1) {
                if ((i) % block_size == 0) {
                    r = 0;
                    block = util.CreateBlock(block_size);
                    last_block = util.ReadBlock(block, block_size, text, i, padding);
