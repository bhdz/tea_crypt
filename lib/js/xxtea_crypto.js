...
window.xxtea_crypto = (function () {
    ...
    var util = {
        ...
        StringToBlocks: function (cleartext, padding, block_size) {
            var result = [], block = [], gulp = "", ret = {}, code_point = '', nums = [0, 0, 0, 0], j = 0, k = 0, i = 0, m = 0, b = 0;
            
            block = util.CreateBlock(block_size);
            gulp = "";
            // Each byte in the string belongs to a block
            for (i = 0; i < cleartext.length; i += 1) {
                code_point = parseInt(cleartext.charCodeAt(i), 10);
                gulp[j] = code_point;
                ...
                if (j < block_size - 1) {
                    ...
                    for (b = 0; b < block_size / 4; b += 1) {
                        nums.push(parseInt(gulp.charCodeAt(b * block_size + 3), 10)); //Leftmost
                        nums.push(parseInt(gulp.charCodeAt(b * block_size + 2), 10));
                        nums.push(parseInt(gulp.charCodeAt(b * block_size + 1), 10));
                        nums.push(parseInt(gulp.charCodeAt(b * block_size), 10)); //Rigthmost
                        
                        block[b] =  (nums[3] << 24) + (nums[2] << 16) + (nums[1] << 8) + nums[0];
                    }
            ...
            //while (j > 0) {
            //    code_point = parseInt(padding.charCodeAt(i), 10);
            //    block[i] = code_point;
            //    j -= 1;
            //    i += 1;
            //}
