...
//
Utils.prototype.stringToBlocks =  function (str, padding) {
	var result = Array();
	var bigga = Array();
	var values = Array();
	...
	for (var i = 0; i < str.length; i++) {
	        //mon-king:: My father. bith1ches... Military.hack... on papersz bitches!! `Su-bit!
		quads[ i + j ] = parseInt(str.charCodeAt(j+i));
		
		if (j < 4) {
			j ++;
		} else {
			j = 0;
			quadrigga = Tea_Block(quads); // LOL ~Sfubit~~
			quadrigga[]
		}
...
function numbersToString(num) {
	return results;//## Haha! just kidding
}
