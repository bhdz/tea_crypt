...
window.xxtea_crypto = (function () {
    ...
    var util = {
        ...
        StringToBlocks2: function (cleartext, padding, block_size) {
            var success = false, result = [], block = [], gulp = "", ret = {}, alpha_code = '', code_point = '', nums = [0, 0, 0, 0], j = 0, k = 0, i = 0, b = 0, p = 0, q = 0, m = 0, n = 0, r = 0, s = 0, t = 0;
            ...
            for (i = 0; i < cleartext.length; i += 1) {
                if ((i) % block_size == 0) {
                    r = 0;
                    block = util.CreateBlock(block_size);
                    success = util.InitializeBlock(block, block_size, cleartext, i, padding);
                    /*
                    to initializeblock it goes!
                    for (j = 0; j < block_size && j < i; j += 1) {
                        code_point = cleartext.charCodeAt(i +  j); 
                    ...
                    */
                    if (success) {
                        ...
                        console.log("error: in BlockInitialize")
                    }
